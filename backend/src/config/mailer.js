...
/**
 * Envía un recordatorio al paciente para tomar su medicamento.
 */
const sendMedicationReminder = async ({ to, nombre, medicamento, dosis, horario, instrucciones }) => {
  if (!transporter) {
    const testAccount = await nodemailer.createTestAccount();
    transporter = nodemailer.createTransport({
      host: 'smtp.ethereal.email',
  ...
  const info = await transporter.sendMail({
    from,
    to,
    subject: 'Recordatorio: Es hora de tomar tu medicamento - EPS',
    text: `Hola ${nombre},\n\n¡Es hora de tomar ${medicamento} ${dosis}!\n\nHorario: ${horario}${instrucciones ? `\n\nInstrucciones: ${instrucciones}` : ''}\n\nEPS Portal del Afiliado`,
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <div style="background: linear-gradient(135deg, #8B5CF6 0%, #06B6D4 100%); border-radius: 16px; padding: 24px; text-align: center; margin-bottom: 24px;">
          <h1 style="color: white; margin: 0; font-size: 20px;">EPS — Portal del Afiliado</h1>
        </div>
        <h2 style="color: #1e293b; font-size: 18px;">💊 Recordatorio de medicamento</h2>
        <p style="color: #64748b;">Hola <strong>${nombre}</strong>,</p>
        <div style="background: #F5F3FF; border: 2px solid #C4B5FD; border-radius: 12px; padding: 20px; text-align: center; margin: 24px 0;">
          <p style="color: #7C3AED; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 8px 0;">¡Es hora de tomar!</p>
          <p style="color: #1e293b; font-size: 22px; font-weight: 800; margin: 0;">${medicamento} <span style="color: #7C3AED;">${dosis}</span></p>
          <p style="color: #94a3b8; font-size: 13px; margin: 8px 0 0 0;">Horario programado: <strong>${horario}</strong></p>
        </div>
        ${instrucciones ? `
        <div style="background: #ECFEFF; border: 1px solid #A5F3FC; border-radius: 12px; padding: 16px; margin: 20px 0;">
          <p style="color: #64748b; font-size: 13px; margin: 0 0 6px 0; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Instrucciones</p>
          <p style="color: #1e293b; font-size: 14px; margin: 0;">${instrucciones}</p>
        </div>
        ` : ''}
        <p style="color: #94a3b8; font-size: 13px;">Puedes registrar la dosis tomada desde tu portal del afiliado.</p>
        <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
        <p style="color: #cbd5e1; font-size: 12px; text-align: center;">EPS — Portal del Afiliado</p>
      </div>
    `,
...
module.exports = { sendRecoveryEmail, sendAppointmentConfirmation, sendConsultaCompletada, sendRenewalResult, sendAuthorizationResult, sendMedicationReminder, isConfigured };
