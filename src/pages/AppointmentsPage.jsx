...
import {
  Calendar, CalendarPlus, Filter, Stethoscope, User, MapPin,
  Clock, MoreVertical, X as XIcon, AlertTriangle, CalendarClock
} from 'lucide-react';
...
import { formatDateShort, formatTime, formatDateFull } from '../utils/formatters';
...
const AppointmentsPage = () => {
  ...
  // Unique specialties from appointments for the specialty filter
  ...
  const canReschedule = (apt) => {
    return canModify(apt) && (apt.reagendamientos || 0) < MAX_RESCHEDULES;
  };
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {sorted.map(apt => (
            <Card key={apt.id} hover className="group">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 sm:flex-shrink-0">
                  <Button variant="ghost" size="sm" onClick={() => setDetailModal(apt)}>
                    Ver detalles
                  </Button>
                  {canReschedule(apt) && (
                    <Button variant="outline" size="sm" className="text-primary-600 border-primary-200 hover:bg-primary-50"
                      icon={<CalendarClock size={15} />}
                      onClick={() => { setRescheduleModal(apt); setRescheduleDate(''); setRescheduleTime(''); setAvailableTimes([]); }}>
                      Reagendar
                    </Button>
                  )}
                  {canModify(apt) && (
